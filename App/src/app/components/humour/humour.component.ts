...
export class HumourComponent implements OnInit {
  ...
  PrevPagination() {
    ...
    this.service.getMethod(`http://skunkworks.ignitesol.com:8000/books/?page=${this.prevPage}&topic=humour`).subscribe((data: any) => {
      this.result = data.results
    })
    if (this.prevPage === 1) {
      this.prevPage = 1;
  ...
  pagination() {
    this.service.getMethod(`http://skunkworks.ignitesol.com:8000/books/?page=${this.mainPage}&topic=humour`).subscribe((data: any) => {
      this.result = data.results
    })
  }
  ...
  nextPagination() {
    ...
    this.nextpage = this.mainPage + 1
    this.service.getMethod(`http://skunkworks.ignitesol.com:8000/books/?page=${this.nextpage}&topic=humour`).subscribe((data: any) => {
      this.result = data.results
    });
  ...
  bookTextTormat(id: any) {
    this.service.getMethod(`http://skunkworks.ignitesol.com:8000/books/?ids=${id}&topic=humour`).subscribe((data: any) => {
